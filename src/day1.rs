...
type TRANSLATIONS = [(&'static str, u32)];

fn line_nums<'a>(s: &'a str, translate: &'a TRANSLATIONS) -> impl Iterator<Item = u32> + 'a {
    (0..s.len()).filter_map(move |start| {
        let substr = &s[start..];
        for (text, num) in translate {
            if substr.starts_with(text) {
                return Some(*num);
            }
        }
        substr.chars().next().and_then(|c| c.to_digit(10))
    })
}

fn calibrate(s: &str, translate: &TRANSLATIONS) -> u32 {
    s.lines()
        .map(|line| {
            let mut nums = line_nums(line, translate).peekable();
            return (nums.peek().unwrap() * 10) + nums.last().unwrap();
        })
        .sum()
}
...
pub fn day1() {
    ...
    println!("{}", calibrate(INPUT, &NUMBERS));
}
...
const INPUT: &str = "dqfournine5four2jmlqcgv
7ggzdnjxndfive
twofive4threenine
